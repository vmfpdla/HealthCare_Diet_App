  ...
  <?php
    ...
    if ($result2 -> num_rows>0) { // 여러줄 가져오는 경우
      ...
      while($row = $result2->fetch_assoc()) {
        ...
        $car = $car + 0.5*$row['food_car'];
        $pro = $pro + 0.5*$row['food_pro'];
        $fat = $fat + 0.5*$row['food_fat'];
        }

        else if($row['eaten_serving']==1){ # 0인분인경우
        ...
        else if($row['eaten_serving']==2){ # 0인분인경우
...
<div class="card">
  ...
  <div class="card-body" id="camera_1"  >
...
<div class="card">
  ...
  <div class="card-body" id="camera_2"  >
    <i class="fas fa-utensils" aria-hidden="true"></i>
  </div>
